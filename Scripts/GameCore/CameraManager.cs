    ...
    public class CameraManager : MonoSingleton<CameraManager>
    {
        ...
        private CinemachineVirtualCamera CurrentVirtualCamera
        {
            get
            {
                ...
                {
                    Debug.LogWarning("Can not get virtual camera");
                }
        ...
        protected override void Awake()
        {
            ...
            _cineBrain.m_CameraActivatedEvent.AddListener(UpdateCurrentVirtualCamera);
        }

        public void UpdateCurrentVirtualCamera(ICinemachineCamera newCam, ICinemachineCamera oldCam)
        {
            if (oldCam != null)
            {
                // oldCam.Priority = 0;
            ...
            {
                newCam.Priority = 1;
                newCam.Follow = PlayerManager.Instance.GetPlayerHolderTransform();
                CurrentVirtualCamera = newCam as CinemachineVirtualCamera;                
            }
            else
            {
                Debug.Log("Current Transition to a null vcam");
            }

        }
        public void SetVirtualCameraTarget(Transform targetTransform)
        {
            CurrentVirtualCamera.Follow = targetTransform;
        }

        public void SetCurrentVirtualCameraPriority(int targetValue)
        {
            CurrentVirtualCamera.Priority = targetValue;
        }
